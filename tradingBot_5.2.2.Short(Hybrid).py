...
def GetNewTrades():
    ...
    for a in range(len(positions)):
        ...
        """Ticket, Symbol, Volume, Open Price, SL, Low"""
        ...
        if result.retcode != mt5.TRADE_RETCODE_DONE:
                print("{} SL failed to modify in GetNewTrades for ticket {} {}, retcode={}, SL {}, ".format(datetime.now().strftime("%H:%M:%S"),result.deal,positions[a].symbol,result.retcode,newPos[4]))
        else:
...
def market_order(symbol,volume,sl):
    ...
    if result.retcode == mt5.TRADE_RETCODE_DONE:
        ...
        print("{} Invalid SL on Symbol {}".format(datetime.now().strftime("%H:%M:%S"),symbol))
         ...
         print("{} Invalid price {} on Symbol {}".format(datetime.now().strftime("%H:%M:%S"),sl,symbol))

    else:
        print("{} ORDER FAILED retcode={} {} request in short {} ".format(datetime.now().strftime("%H:%M:%S"),result.retcode,symbol,request))

    return result

def OpenTrades():
     for S in range(len(SYMBOL)):
        symPrice = mt5.symbol_info_tick(SYMBOL[S]).bid
        ...
        for i in range(len(List)):
            ...
            for x in range(4):
                ...
                if List[i][x][3] >= symPrice:
                    #Open position
                    print('Position price reached, Opening position')
                    ...
                    for z in range(x-1,-1,-1):
                        ...
                        if result.retcode != mt5.TRADE_RETCODE_DONE:
                                print("{} SL failed to modify in OpenTrades for ticket {} {}, retcode={}, SL {}, ".format(datetime.now().strftime("%H:%M:%S"),result.deal,SYMBOL[S],result.retcode,List[i][x][4]))
                        else:
                                print('{} SL modified with Ticket {} {} and SL {}'.format(datetime.now().strftime("%H:%M:%S"),result.deal,SYMBOL[S],List[i][x][4]))
...
def ModifySL6():   
    ...
    for i in range(len(List)):
        ...
        for x in range(4):
            ...
            if result.retcode != mt5.TRADE_RETCODE_DONE:
                    print("{} SL failed to modify in ModifySL6 for ticket {} {}, retcode={}, SL {}, ".format(datetime.now().strftime("%H:%M:%S"),result.deal,List[i][x][1],result.retcode,List[i][0][3] - 60 * point))
            else:
                    print('{} New asi ma vyjebe trade added with Ticket {} {} and SL {}'.format(datetime.now().strftime("%H:%M:%S"),result.deal,List[i][x][1],List[i][0][3] - 60 * point))
    ...
    for S in range(len(SYMBOL)):
        for i in range(len(List)):
            ...
            for x in range(4):
                ...
                if result.retcode != mt5.TRADE_RETCODE_DONE:
                    print("{} SL failed to modify in modifySLhigh for ticket {} {}, retcode={}, SL {}, ".format(datetime.now().strftime("%H:%M:%S"),List[i][x][0],SYMBOL[S],result.retcode,lastHigh))
                else:
...
path = 'C:\TradingBot\Log5-2-2(Hybrid)\Log_'+str(strftime("%m_%Y", gmtime()))+'.txt'
