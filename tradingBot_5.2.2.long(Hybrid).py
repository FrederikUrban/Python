...
def market_order(symbol,volume,sl):
    ...
    if result.retcode == mt5.TRADE_RETCODE_DONE:
        ...
        print("{} ORDER FAILED retcode={} {} request {}".format(datetime.now().strftime("%H:%M:%S"),result.retcode,symbol,request))
            ...
            for x in range(4):
                ...
                if List[i][x][3] >= symPrice:
                    ...
                    for z in range(x-1,-1,-1):
                        ...
                        if result.retcode != mt5.TRADE_RETCODE_DONE:
                                print("{} TP failed to modify for previous positions, for ticket {} {}, retcode={}, TP {}, ".format(datetime.now().strftime("%H:%M:%S"),result.deal,SYMBOL[S],result.retcode,List[i][x][4]))
                        else:
    ...
    for S in range(len(SYMBOL)):
        for i in range(len(List)):
            ...
            for x in range(4):
                ...
                if result.retcode != mt5.TRADE_RETCODE_DONE:
                    print("{} TP faild to modify for ticket {} {}, retcode={}, SL {}, ".format(datetime.now().strftime("%H:%M:%S"),List[i][x][0],SYMBOL[S],result.retcode,lastHigh))
                else:
