    ...
    for S in range(len(SYMBOL)):
        for i in range(len(List)):
            ...
            for x in range(4):
                ...
                if result.retcode != mt5.TRADE_RETCODE_DONE:
                    print("{} SL or TP faild to modify for ticket {} {}, retcode={}, SL {}, ".format(datetime.now().strftime("%H:%M:%S"),result.deal,SYMBOL[S],result.retcode,lastHigh))
                else:
                    print('{} New trade added with Ticket {} {} and SL {}'.format(datetime.now().strftime("%H:%M:%S"),result.deal,SYMBOL[S],lastHigh))
