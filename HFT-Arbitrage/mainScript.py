...
################# Binance Order Function #################

# Assume BINANCE_API_KEY and BINANCE_API_SECRET are defined elsewhere.
# Also assume you have a logger configured (e.g., using the logging module).

async def place_binance_order(params: dict) -> Optional[dict]:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            logger.info(f"Sending binance order")
            
            async with session.post(
                url,
                data=params,
                headers=headers,
                timeout=aiohttp.ClientTimeout(total=60) # Max waiting time is 60s 
            ) as response:
                
                data = await response.json(loads=orjson.loads)
                    # Record and log the time when the response is received.
                logger.info(f"Received binance response for order")
                
                # Check if the response indicates a successful order placement.
                if response.status == 200:
                    logger.info(f"Binance Order Executed: {data}")
                    # Continue with the next steps in your operations.
                    return data
                else:
                    logger.warning(f"Order did not complete as expected: {data}")
    ...
    # If all attempts fail, return None or handle the failure accordingly.
    return None
...
async def closeLS(position, diff, bybit_client):
    """Closes LS (Long Sell) positions in a structured parallel manner."""
    qtyToClose = open_trades_LS[position]['cnt']

    async with asyncio.TaskGroup() as tg:
        tg.create_task(place_binance_order({"symbol": "ADAUSDT", "type": "MARKET", "side": "SELL", "positionSide": "LONG", "quantity": qtyToClose}))  # Close Binance long
        tg.create_task(place_binance_order({"symbol": "ADAUSDT", "type": "MARKET", "side": "BUY", "positionSide": "LONG", "quantity": qtyToClose}))  # Reopen Binance long
...
async def closeSL(position, diff, bybit_client):
    """Closes SL (Short Long) positions using TaskGroup."""
    qtyToClose = open_trades_SL[position]['cnt']

    async with asyncio.TaskGroup() as tg:
        tg.create_task(place_binance_order({"symbol": "ADAUSDT", "type": "MARKET", "side": "BUY", "positionSide": "SHORT", "quantity": qtyToClose}))  # Close Binance short
        tg.create_task(place_binance_order({"symbol": "ADAUSDT", "type": "MARKET", "side": "SELL", "positionSide": "SHORT", "quantity": qtyToClose}))  # Reopen Binance short
...
async def openPos(diff,bybit_client):
    """Opens positions in parallel using TaskGroup for efficiency."""
    async with asyncio.TaskGroup() as tg:
        tg.create_task(place_binance_order({"symbol": "ADAUSDT", "type": "MARKET", "side": "BUY", "positionSide": "LONG", "quantity": QTY}))  # Open Binance long
        tg.create_task(place_binance_order({"symbol": "ADAUSDT", "type": "MARKET", "side": "SELL", "positionSide": "SHORT", "quantity": QTY}))  # Open Binance short
...
################# Trading Engine #################
async def trading_strategy(price_update_event, close_event, bybit_client: BybitClient,):
...
async def calcPosToClose(ls: dict,sl: dict, close_event, bybit_client):
    while True:
        ...
        try:
            ...
            if count > MAXCOUNT:
                ...
                async with asyncio.TaskGroup() as tg:
                    tg.create_task(place_binance_order({"symbol": "ADAUSDT", "type": "MARKET", "side": "SELL", "positionSide": "LONG", "quantity": qtyToClose}))  # Close Binance long position
                    tg.create_task(bybit_client.place_order("Buy", qtyToClose, "ADAUSDT", 2))  # Close Bybit short
                    tg.create_task(place_binance_order({"symbol": "ADAUSDT", "type": "MARKET", "side": "BUY", "positionSide": "SHORT", "quantity": qtyToClose}))  # Close Binance short position
...
async def main():
    ...
    binance_ws = None
    # Initialize the working Bybit client.
    ...
    try:
        ...
        tasks = [
            loop.create_task(binance_price_producer(binance_ws, price_update_event)),
            loop.create_task(bybit_price_producer(price_update_event)),
            loop.create_task(price_consumer()),
            loop.create_task(trading_strategy(price_update_event, close_event, bybit_client)),
            loop.create_task(display_prices()),
            loop.create_task(calcPosToClose(open_trades_LS, open_trades_SL, close_event, bybit_client))
        ]
