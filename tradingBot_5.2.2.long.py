...
def initialize(Login,Password,Server):
    ...
    if not mt5.initialize("C:/Program Files/FTMO MetaTrader 5 - Copy (2)/terminal64.exe"):
...
def GetNewTrades():
    ...
    for a in range(len(positions)):
        ...
        if result.retcode != mt5.TRADE_RETCODE_DONE:
                print("{} SL failed to modify for ticket {} {}, retcode={}, SL {}, ".format(datetime.now().strftime("%H:%M:%S"),result.deal,positions[a].symbol,result.retcode,newPos[4]))
        else:
            ...
            for x in range(4):
                ...
                if List[i][x][3] >= symPrice:
                    ...
                    for z in range(x-1,-1,-1):
                        ...
                        if result.retcode != mt5.TRADE_RETCODE_DONE:
                                print("{} SL or TP failed to modify for ticket {} {}, retcode={}, SL {}, ".format(datetime.now().strftime("%H:%M:%S"),result.deal,SYMBOL[S],result.retcode,List[i][x][4]))
                        else:
...
def ModifySL6():   
    ...
    for i in range(len(List)):
        ...
        for x in range(4):
            ...
            if symInfo[0].tp < List[i][0][3] - 60*point:
                break
    ...
    for S in range(len(SYMBOL)):
        for i in range(len(List)):
            ...
            for x in range(4):
                ...
                if lastHigh > posInfo[0].tp - 10*point:
                    break
                ...
                if result.retcode != mt5.TRADE_RETCODE_DONE:
                    print("{} SL failed to modify for ticket {} {}, retcode={}, SL {}, ".format(datetime.now().strftime("%H:%M:%S"),List[i][x][0],SYMBOL[S],result.retcode,lastHigh))
                else:
...
path = 'C:\TradingBot\Log5-2-2-long\Log_'+str(strftime("%m_%Y", gmtime()))+'.txt'
