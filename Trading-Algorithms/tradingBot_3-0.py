...
def initialize():
    ...
    if not mt5.initialize("C:/Program Files/Traders Global Group MetaTrader 5/terminal64.exe"):
...
def longOrShort():
    ...
    if last_open == last_close:
        print("last_open == last_close on Symbol {}, Open {}, Close{}, Time -> {}".format(SYMBOL[S],last_open,last_close,datetime_kyjev.strftime("%H:%M:%S")))
        return None
    ...
    return True if last_open < last_close else False
...
def isGoingUp(LENGHT,TIMEFRAME):
    ...
    bars = mt5.copy_rates_from_pos(SYMBOL[S], TIMEFRAME, 4, LENGHT)
    bars_df = pd.DataFrame(bars) 
...
maxDailyLoss = -240
active = [False] * len(SYMBOL)
buy = [False] * len(SYMBOL)
buyList = []
sellList = []
console = sys.stdout
...
while True:
    ...
    for S in range(len(SYMBOL)):
        if active[S] == True:
            ...
            path = 'C:\TradingBot\Log3-0\Log_'+str(strftime("%d_%b_%Y", gmtime()))+'_'+SYMBOL[S]+'.txt'
            ...
            if LOR[S] and SMA50[S] and SMA200[S] and checkSMA[S]:
                ...
                #append to buyList
                buyList.append(SYMBOL[S])
                active[S] = False
                     

            #Short
            elif not LOR[S] and not SMA50[S] and not SMA200[S] and not checkSMA[S]:
                
                #append to sellList
                sellList.append(SYMBOL[S])
                active[S] = False

            else:
                print("No trade on Symbol {}, Diff {}, Time -> {}".format(SYMBOL[S],diff[S],datetime_kyjev.strftime("%H:%M:%S")))
                active[S] = False   
...
#Check for 'q' key
    if keyboard.is_pressed('q'):
        #swtich to console
        sys.stdout = console
        print("buyList: {}".format(buyList))
        print("sellList: {}".format(sellList))
  
        while True:
            #sym to upper case
            sym = input("Want make order?\nEnter Symbol\nWanna exit? Enter 'q'\n")
            sym = sym.upper()

            #check if sym is in buyList
            if sym in buyList:
                for S in range(len(SYMBOL)):
                    #find S
                    if sym == SYMBOL[S]:

                        barz = mt5.copy_rates_from_pos(SYMBOL[S], 15, 0, 10)
                        barz_df = pd.DataFrame(barz)
                        ...
                        order[S] = 0
                        SL[S] = truncate(barz_df.iloc[-1].close - diff[S],5)
                        TP[S] = truncate(barz_df.iloc[-1].close + diff[S]*2,5)
                        ...
                        #del from buyList
                        buyList.remove(sym)
                        print("Deleted ",sym,"\nbuyList: {}\nsellList {}".format(buyList,sellList))


            elif sym in sellList:
                for S in range(len(SYMBOL)):
                    #find S
                    if sym == SYMBOL[S]:
                ...
                print("Didnt find Symbol in sellList" + sym)
