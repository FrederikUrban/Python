...
def get_sma7():
    ...
    bars_df = pd.DataFrame(bars)
    SMA7 = bars_df.open.mean()
    return SMA7

def get_sma7minus1():
    bars = mt5.copy_rates_from_pos(SYMBOL, TIMEFRAME, 1, 7)
    bars_df = pd.DataFrame(bars)
    SMA7minus1 = bars_df.open.mean()
    return SMA7minus1
...
while True:
    for S in range(len(SYMBO)):
        ...
        if len(positions) > 0:
            ...
            SMA = get_sma()
            SMA7 = get_sma7()
            SMA7minus1 = get_sma7minus1()
            signal_SMA = check_SMA7()
            signal1,pattern = find_3_candles_above_sma()
            spreadIsLow = check_spread()

            if signal1 == 'buy' and act_open[S] != actual_open and spreadIsLow and signal_SMA:
                find_stopLoss()
                market_order()
                print_everything()
                take_profit[S] = 0.0
